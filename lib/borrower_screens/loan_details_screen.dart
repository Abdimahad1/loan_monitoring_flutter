...
import 'package:loan_monitoring_flutter/borrower_screens/payments_screen.dart';
...
class _LoanDetailsScreenState extends State<LoanDetailsScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadLoanDetails() async {
    ...
    try {
      print('🔄 Loading loan details for ID: ${widget.loanId}');
      final result = await _apiService.getLoanDetails(widget.loanId);

      if (mounted) {
        if (result['success'] == true) {
          final loan = result['loan'] as Map<String, dynamic>;

          // Debug print to see what we're getting
          print('📊 Loan data received:');
          print('  - Loan ID: ${loan['loanId']}');
          print('  - Status: ${loan['status']}');
          print('  - Paid Amount from DB: ${loan['paidAmount']}');
          print('  - Remaining from DB: ${loan['remainingAmount']}');

          // Fetch payment history to calculate total paid amount
          ...
          try {
            ...
            if (paymentsResult['success']) {
              ...
              // Filter payments for this loan and calculate total
              for (var payment in payments) {
                if (payment is Map<String, dynamic>) {
                  final paymentLoanId = payment['loanId']?.toString();
                  final paymentLoanDisplayId = payment['loanId_display']?.toString();
                  final loanDisplayId = loan['loanId']?.toString();

                  // Check if this payment belongs to our loan
                  if (paymentLoanId == widget.loanId || paymentLoanDisplayId == loanDisplayId) {
                    print('  💰 Found payment: ${payment['amount']} - ${payment['status']}');

                    if (payment['status']?.toString().toLowerCase() == 'success') {
                      final amount = _toDouble(payment['amount']);
                      totalPaidAmount += amount;
                      print('    ✅ Adding to paid amount: $amount (total now: $totalPaidAmount)');
                    }
          ...
          if (loan['schedule'] != null && loan['schedule'] is List) {
            ...
            // Calculate installment amount (should be total/term = 400/12 = 33.33)
            final totalAmount = _toDouble(loan['amount']);
            final term = scheduleList.length;
            final installmentAmount = totalAmount / term;

            // Calculate how many installments are paid based on total paid amount
            // If totalPaidAmount = 33.33, then paidInstallmentsCount should be 1
            final paidInstallmentsCount = (totalPaidAmount / installmentAmount).round();
            ...
            for (int i = 0; i < scheduleList.length; i++) {
              ...
              if (inst is Map) {
                ...
                // If we have paid installments remaining, mark this one as paid
                if (paidSoFar < paidInstallmentsCount) {
                  status = 'paid';
                  paidSoFar++;
                  print('✅ Installment #${i + 1} (index $i) marked as PAID');
                } else {
                  // Otherwise use the original status or pending
                  status = inst['status']?.toString().toLowerCase() ?? 'pending';
                  print('📅 Installment #${i + 1} (index $i) marked as $status');
                }

                processedSchedule.add({
                  'installmentNo': i + 1, // Display number starts from 1
            ...
            // Verify which installments are paid
            for (var inst in processedSchedule) {
              if (inst['status'] == 'paid') {
                print('  - Installment #${inst['installmentNo']} is PAID');
              }
            }
          }

          // Sort payments by date (newest first) for display
          ...
          print('✅ Successfully loaded loan details');
    ...
    return Scaffold(
      ...
      floatingActionButton: canPay
        ...
        child: FloatingActionButton.extended(
          ...
          label: Text(
            status == 'overdue' ? 'Pay Overdue' : 'Make Payment',
            style: const TextStyle(
              color: Colors.white,
              fontWeight: FontWeight.w600,
            ),
      ...
      itemBuilder: (context, index) {
        ...
        return Padding(
          ...
          child: InstallmentTile(
            installmentNo: installment['installmentNo'] ?? index + 1, // Use stored installmentNo
            dueDate: dueDate,
